...
async def list_guidelines(
    ...
    if q:
        items, total = await service.search(q, offset=params.offset, limit=params.page_size)
    else:
        items, total = await service.get_list(params, department=department)
    ...
    return Response.ok({"items": [{"id": str(g.id), "title": g.title,
                                   ...
                                   "summary": g.summary,
                                   "disease_tags": g.disease_tags} for g in items],
...
async def get_guideline(guideline_id: UUID, session: AsyncSession = Depends(get_db)):
    ...
    return Response.ok({"id": str(g.id), "title": g.title, "content": g.content,
                         ...
                         "organization": g.organization, "publish_year": g.publish_year,
                         "department": g.department, "disease_tags": g.disease_tags})
